...
_DETAIL_FILENAME_BY_KEY = {"kq": "foi-kq.html"}
_KQ_FIELD_HEADERS = ("bcvp", "k1", "k2", "q1", "q2")


def write(fois, json_output_path):
    """Write HTML pages for the FOI catalog."""
    _write_index(fois, json_output_path)
    if kq_catalog := fois.get("kq"):
        _write_kq_page(kq_catalog, json_output_path)


def _write_index(fois, json_output_path):
    ...
    body_contents = _body_wrapper(
        [
            ...
            my_html.para(_json_link_contents(json_output_path)),
...
def _json_link_contents(json_output_path):
    return [
        "JSON catalog: ",
        my_html.anchor(
            "features_of_interest.json", {"href": _href_to_json(json_output_path)}
        ),
    ]


def _href_to_json(json_output_path):
    return f"../../{json_output_path}"
