...
The 2 functions are book39_name & mam_book_path.
A BS_.* pair is a book/sub-book pair.
The 1st part of a BS_.* pair is a book24 string.
...
# this type as a "book39", and we sometimes refer to the other kind of book as
# a "book24".
#
...
def book39_name(bk24na, sub_bkna):
    """
    Given book24 name bk24na & sub-book name sub_bkna, return the
    corresponding book39 name. Output and both inputs are expected to be
    Hebrew.
    """
    if sub_bkna is None:
        return _strip_sfr_or_mgylt(bk24na)
    if bk24na in ('ספר תרי עשר', 'ספר עזרא'):
        # Unfortunately Ovadiah is spelled ḥaser-style (without a vav)
