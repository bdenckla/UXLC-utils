...
def mk_dic_for_shifted_spans(ss_recs):
    ...
    for shift_rec in ss_recs:
        ...
        bkid, chnu, span_start_v = tbn.bcvt_get_bcv_triple(span_start_bcvt)
        ...
        for span_v in range(span_start_v, span_start_v + span_len):
            mam_v = span_v + shift
            bcvtmam = tbn.mk_bcvtmam(bkid, chnu, mam_v)
...
def mk_maprec_1_to_1_and_empties(cv_nonempty, cv_empty1, cv_empty2):
    ...
    """
    return (
        _mk_cvve_f(cv_nonempty),
        _mk_cvve_e(cv_empty1),
        _mk_cvve_e(cv_empty2)
    )
