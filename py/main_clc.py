...
Usage (run from the repo root, like the other py/main_*.py drivers):
    ...
    python py/main_clc.py [BookId]      # default: Proverbs
...
step 5). Pass any bk39 id (e.g. "Psalms", "Job", "Genesis") to retarget.

Writes:
    gh-pages/clc/<book>.html        the 3-column always-link page
    gh-pages/clc/<book>-notes.json  the CLC notes as plain data (feeds §7.9 later)
"""
...
def main():
    ...
    book, notes = clc_collect.collect_for_book(book_id)
    html_path = clc_render.write_book(book_id, book, notes)
    notes_path = f"gh-pages/clc/{book_id}-notes.json"
    my_open.json_dump_to_file_path([note.as_dict() for note in notes], notes_path)
    print(f"CLC skeleton: {len(notes)} notes for {book_id}")
